...
export function VerificationTimeline({ status = "pending", submittedAt = "", updatedAt = "" }) {
  ...
  useEffect(() => {
    ...
    // 3. Staggered Timeline Sequence (Total duration ~ 600ms for high responsiveness)
    // 0ms -> Step 1 Active ("Details Submitted")
    const t1 = setTimeout(() => setStep1State("active"), 50);

    // 300ms -> Step 1 Done & Step 2 Active ("Verification Status")
    const t2 = setTimeout(() => {
      setStep1State("done");
      setStep2State("active");
    }, 300);

    // 600ms -> Step 2 Done (Resolved visual state triggers)
    const t3 = setTimeout(() => {
      setStep2State("done");
    }, 600);
      ...
      case "verified":
        return {
          circleClass: `border-2 border-[#22C55E] bg-[#22C55E] text-white ${
            step2State === "done" && !skipAnim ? "animate-ripple-green" : ""
          }`,
          label: "Verification Approved",
          labelClass: "text-[#22C55E] font-bold",
          icon: <AnimatedCheckIcon active={step2State === "done"} />,
        };
      ...
      case "resubmission_required":
        return {
          circleClass: `border-2 border-[#EF4444] bg-[#FEF2F2] text-[#EF4444] ${
            step2State === "done" && !skipAnim ? "animate-shake-red" : ""
          }`,
      ...
      default:
        return {
          circleClass: `border-2 border-amber-500 bg-amber-50/50 text-amber-600 ${
            step2State === "done" ? "animate-pulse-amber" : ""
          }`,
          ...
          icon: (
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-[15px] h-[15px]">
              ...
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l4 2" className={step2State === "done" ? "animate-spin-slow" : ""} />
            </svg>
          ),
        };
    }
  ...
  return (
    <div className="flex flex-col items-start w-full max-w-[340px] mx-auto mt-6 mb-2 select-none">
      ...
      {/* Step 1 — Details Submitted (Sequenced) */}
      <div 
      ...
      >
        <div className="flex flex-col items-center">
          ...
          >
            {step1State === "done" ? <AnimatedCheckIcon active={!skipAnim && step1State === "done"} /> : null}
          </div>
          <div 
      ...
      {/* Step 2 — Dynamic Verification Status */}
      <div 
        className={`flex items-start gap-5 w-full relative transition-all duration-300 ${
      ...
      >
        <div className="flex flex-col items-center">
          ...
          >
            {step2State === "done" ? step2.icon : null}
          </div>
        </div>
        <div className="pt-1.5 flex flex-col">
          <span 
            className={`text-[14.5px] tracking-wide transition-colors duration-300 ${
              step2State === "done" ? step2.labelClass : "text-[#9CA3AF] font-medium"
            }`}
          >
            {step2State === "done" ? step2.label : "Verification Status"}
          </span>
          {step2State === "done" ? (
