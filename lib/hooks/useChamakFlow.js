...
import { triggerAnalysis, triggerGenerate, CHAMAK_PIPELINES } from "../api/chamak";
import {
  createGeneration,
  fetchGeneration,
  fetchGallery,
...
// Chamak 2.0 renders on OpenAI, which is materially slower than Nanobana —
// measured medians sit around 33s with a long tail, and 2K output is slower
// still. Polling on 1.0's ~100s budget would report a successful generation
// as failed, so 2.0 gets its own ceiling. Set Creation hit exactly this and
// had to do the same (see useSetCreationFlow).
const OPENAI_POLL_INTERVAL_MS = 3000;
const OPENAI_MAX_POLL_ATTEMPTS = 80; // ~240 seconds

export function useChamakFlow(wholesalerId, userId, pipeline = CHAMAK_PIPELINES.NANOBANA) {
  const effectiveUserId = userId || wholesalerId;
  const isOpenAIPipeline = pipeline === CHAMAK_PIPELINES.OPENAI;
  const pollIntervalMs = isOpenAIPipeline ? OPENAI_POLL_INTERVAL_MS : POLL_INTERVAL_MS;
  const maxPollAttempts = isOpenAIPipeline ? OPENAI_MAX_POLL_ATTEMPTS : MAX_POLL_ATTEMPTS;
  ...
  const startPolling = useCallback(
    ...
    [stopPolling, effectiveUserId, refreshCredits, pollIntervalMs, maxPollAttempts]
  );
  ...
  const submitFormAndGenerate = useCallback(async () => {
    ...
    try {
      ...
      //    `pipeline` picks the renderer — Nanobana (1.0) or OpenAI (2.0).
      await triggerGenerate(updatedGen.id, pendingGenerateKeyRef.current, pipeline);
  ...
  }, [currentGeneration, sliderValues, noteText, effectiveUserId, startPolling, wallet, pipeline]);
