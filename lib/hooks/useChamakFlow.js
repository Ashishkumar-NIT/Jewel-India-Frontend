...
import { useUploadUsage } from "./useUploadUsage";
...
export function useChamakFlow(wholesalerId, userId) {
  ...
  const { used, limit, isLoading: usageLoading, isLimitReached, refetch: refetchUsage } = useUploadUsage(effectiveUserId);
  ...
  // Quota calculation (Fix #4: Unlimited when limit is null or Infinity)
  const isQuotaUnlimited = limit === null || limit === Infinity;
  const remainingQuota = isQuotaUnlimited ? Infinity : Math.max(0, limit - used);
  const totalQuota = isQuotaUnlimited ? "Unlimited" : limit;
  ...
  const startPolling = useCallback(
    ...
    [stopPolling, effectiveUserId, refetchUsage]
  );
  ...
  const submitFormAndGenerate = useCallback(async () => {
    ...
    if (!isQuotaUnlimited && isLimitReached) {
      setErrorMessage("Daily fusion credit limit reached. Please try again tomorrow.");
      return;
    }
    ...
    try {
      ...
      // 2. Trigger FastAPI AI pipeline for fusion generation
      await triggerGenerate(updatedGen.id);
  ...
  }, [currentGeneration, isQuotaUnlimited, isLimitReached, sliderValues, noteText, effectiveUserId, startPolling]);

  // Revise & Retry (from result or failure screen)
  ...
  const resetToPicker = useCallback(() => {
    ...
    refetchUsage();
    loadData();
  }, [stopPolling, refetchUsage, loadData]);
